...
H = yaml.safe_load((ROOT / "quality-registry" / "harness.yaml").read_text(enquality="utf-8"))
...
def main() -> int:
    ...
    try:
        ...
        got = run(engine,
                  pathlib.Path(a.prompt_file).read_text(enquality="utf-8"),
                  json.loads(pathlib.Path(a.schema).read_text(enquality="utf-8")))
    except EngineError as e:
