#!/usr/bin/env python3
"""Static checks over the quality-* skill set.

Every threshold and every piece of vocabulary comes from quality-registry/harness.yaml.
...
def load(name: str):
    with (ROOT / "quality-registry" / name).open(encoding="utf-8") as fh:
...
def v17_delivery():
    for key, spec in H["delivered"].items():
        want = read(ROOT / "quality-registry" / "delivered" / f"{key}.md").rstrip("\n")
        ...
        for d in SKILL_DIRS:
            ...
            if got != want:
                fail("V17", f"{d.name}/SKILL.md {key} block differs from "
                            f"quality-registry/delivered/{key}.md (run: make render)")
...
def v28_namespace_agrees():
    ...
    # and a sibling set's install line (`cp -R quality-* _common <skills dir>`)
